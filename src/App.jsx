          ...
          {filtered.length === 0
            ...
            : filtered.map(m => (
              <div key={m.id} style={styles.dropdownItem} onClick={() => select(m)}
                ...
                <span style={{ color: "#888", fontSize: 12 }}>¥{m.price} / {m.unit}</span>
              </div>
            ))}
        </div>
      )}
...
export default function App() {
  ...
  const saveCalc = () => {
    ...
    const entry = {
      ...
      totalCost, sellingRaw, selling, taxRate, costRate, grossProfit, targetRate, suggestedPrice,
    };
  ...
  return (
    <div style={styles.root}>
      ...
      <div style={styles.body}>
        ...
        {tab === "saved" && (
          <div>
            <div style={{ marginBottom: 14, fontSize: 13, color: "#6b7280" }}>保存した計算結果：{savedList.length}件</div>
            {savedList.length === 0 && (
              <div style={{ ...styles.card, textAlign: "center", color: "#aaa", padding: "40px 0" }}>
                保存済みの計算はありません。<br />原価計算タブで「💾 この計算を保存」を押してください。
              </div>
            )}
            {savedList.map(entry => (
